...
describe('ManageCommands', () => {
  ...
  let testDir: string;
  let mockInstaller: ICommandInstaller;

  const createPlaceholderInstalledCommand = (): InstalledCommand => ({
    name: 'placeholder',
    package: 'placeholder',
    version: '0.0.0',
  ...
  beforeEach(async () => {
    ...
    mockInstaller = {
      install: vi.fn(async () => createPlaceholderInstalledCommand()),
      uninstall: vi.fn(async () => undefined),
      update: vi.fn(async () => createPlaceholderInstalledCommand()),
      isInstalled: vi.fn(async () => false),
      getCommandsPath: vi.fn(() => testDir),
      loadInstalledCommand: vi.fn(async () => null),
      getManifestPath: vi.fn(() => join(testDir, 'commands-manifest.json')),
    };
  ...
  describe('install action', () => {
    ...
    it('should handle already installed command without force', async () => {
      ...
      mockInstaller.isInstalled = vi.fn().mockResolvedValue(true);
