...
describe.skipIf(!runIntegrationTests)('OAuth + WebSocket Authentication Flow Integration', () => {
  ...
  describe('Complete OAuth + WebSocket Authentication Flow', () => {
    ...
    it('should handle end-to-end message transmission with authentication', async () => {
      ...
      const clientMessage = serverHistory.find(
        (msg) =>
          msg.direction === 'incoming' && 'method' in msg.data && msg.data.method === 'client/test',
      );
      ...
      expect(serverMessageReceived.result.message).toBe('End-to-end authenticated server message');
