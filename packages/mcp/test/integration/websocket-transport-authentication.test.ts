...
describe.skipIf(!runIntegrationTests)('WebSocket Transport Authentication Integration', () => {
  ...
  describe('WebSocket Transport with OAuth Authentication', () => {
    ...
    it('should send and receive real messages over WebSocket connection', async () => {
      ...
      const sentMessage = messageHistory.find(
        (msg) =>
          msg.direction === 'incoming' && 'method' in msg.data && msg.data.method === 'test_method',
      );
      ...
      expect((echoResponse.result as { echo: { method: string } }).echo.method).toBe('test_method');
