...
export class DebuggerSession {
  ...
  public async initialize(): Promise<StartDebugSessionResponse> {
    ...
    if (this.config.breakpoints && this.config.breakpoints.length > 0) {
      const { set } = await this.applyBreakpointMutation({
  ...
  public async runCommand(
    ...
    const mutationResult = await this.applyBreakpointMutation(
      command.breakpoints,
    );
  ...
  private async registerBreakpoint(
    ...
    if (spec.location.url) {
      ...
      if (metadata) {
        try {
          return await this.registerBreakpointForScript(
            metadata,
            spec,
            reference,
          );
  ...
  private async registerBreakpointByUrl(
    spec: BreakpointSpec,
  ): Promise<BreakpointSummary> {
  ...
  private getGeneratedLocation(
    ...
    const direct = this.lookupGeneratedPosition(
      consumer,
      sourceId,
      originalLine,
      originalColumn,
    ...
    const candidates = this.collectGeneratedCandidates(
      consumer,
      sourceId,
      originalLine,
      originalColumn,
  ...
  private resolveSourceIdentifier(
    ...
    if (
      reference.fileUrl &&
      sourceMap.sourcesByFileUrl?.has(reference.fileUrl)
    ) {
  ...
  private async upgradePendingBreakpoints(
    metadata: ScriptMetadata,
  ): Promise<void> {
  ...
  private async upgradeBreakpoint(
    ...
    const sourceId = this.resolveSourceIdentifier(
      metadata.sourceMap,
      upgrade.reference,
    );
    if (!sourceId) {
  ...
  private async createSourceMap(
    ...
    for (const sourceId of consumer.sources) {
      const normalized = this.normalizeSourcePath(
        sourceId,
        sourceRoot,
        scriptDir,
      );
    ...
    return {
      ...
      sourcesByPath,
      sourcesByFileUrl:
        sourcesByFileUrl.size > 0 ? sourcesByFileUrl : undefined,
    };
  }
  ...
  private async parseSourceMap(url: string): Promise<RawSourceMap | undefined> {
    ...
    console.warn(
      `Session ${this.id}: External source map URLs are not supported yet (${url}).`,
    );
    return undefined;
  }
  ...
  private resolveSourceRoot(
    ...
    if (!this.hasUriScheme(sourceRoot)) {
      ...
      return path.normalize(
        path.resolve(this.targetWorkingDirectory, sourceRoot),
      );
    }
    return scriptDir;
  }
  ...
  private fromCdpLocation(location: CdpLocation): BreakpointLocation {
    return {
      scriptId: location.scriptId,
      url:
        this.scripts.get(location.scriptId)?.url ??
        this.scriptUrls.get(location.scriptId),
      lineNumber: location.lineNumber,
      columnNumber: location.columnNumber,
    };
  }
  ...
  private async onScriptParsed(event: ScriptParsedEvent): Promise<void> {
    ...
    if (event.sourceMapURL) {
      try {
        const sourceMap = await this.createSourceMap(
          metadata,
          event.sourceMapURL,
        );
        if (sourceMap) {
