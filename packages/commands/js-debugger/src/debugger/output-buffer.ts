    ...
    public snapshot(): OutputBufferSnapshot {
        return {
            ...
            console: this.entries
                .filter((entry): entry is Extract<OutputEntry, { kind: 'console' }> => entry.kind === 'console')
                .map((entry) => entry.entry),
            exceptions: this.entries
                .filter((entry): entry is Extract<OutputEntry, { kind: 'exception' }> => entry.kind === 'exception')
                .map((entry) => entry.entry),
        };
    }
