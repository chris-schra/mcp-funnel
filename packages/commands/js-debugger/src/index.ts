...
export class JsDebuggerCommand implements ICommand {
    ...
    public async executeToolViaMCP(toolName: string, args: Record<string, unknown>): Promise<CallToolResult> {
        try {
            switch (toolName) {
    ...
    public async executeViaCLI(_args: string[]): Promise<void> {
        console.error('CLI execution is not implemented for js-debugger. Use the MCP interface.');
    }
...
const breakpointLocationSchema = {
    type: 'object',
    properties: {
        scriptId: { type: STRING, description: 'Script identifier reported by CDP.' },
        url: { type: STRING, description: 'URL or absolute path to the script resource.' },
        lineNumber: { type: NUMBER, description: 'Zero-based line number.' },
        columnNumber: { type: NUMBER, description: 'Optional zero-based column number.' },
    },
    required: ['lineNumber'],
} as Tool['inputSchema'];
...
const breakpointSpecSchema = {
    ...
    properties: {
        ...
        condition: { type: STRING, description: 'Optional JavaScript expression evaluated before pausing.' },
    },
    required: ['location'],
} as Tool['inputSchema'];
...
const startSessionSchema = {
    type: 'object',
    properties: {
        id: { type: STRING, description: 'Optional predefined session identifier.' },
        target: {
            type: 'object',
            ...
            properties: {
                ...
                useTsx: { type: BOOLEAN, description: 'Inject `--import tsx/register` for TypeScript debugging.' },
                runtimeArguments: {
                    type: 'array',
                    items: { type: STRING },
                    description: 'Additional Node.js runtime flags (e.g., --trace-warnings).',
                },
                nodePath: { type: STRING, description: 'Path to the Node.js executable.' },
                inspectHost: {
                    type: STRING,
...
const scopeQuerySchema = {
    ...
    properties: {
        sessionId: { type: STRING, description: 'Debugger session identifier.' },
        callFrameId: { type: STRING, description: 'Identifier of the paused call frame.' },
        scopeNumber: { type: NUMBER, description: 'Zero-based index in the scope chain.' },
        path: {
            type: 'array',
...
function parseDebugSessionConfig(input: Record<string, unknown>): DebugSessionConfig {
...
function parseScopeQuery(input: Record<string, unknown>): ScopeQuery {
    ...
    const maxProperties =
        input.maxProperties !== undefined ? expectNumber(input.maxProperties, 'maxProperties') : undefined;
...
function parseBreakpointLocation(value: Record<string, unknown>, label: string): BreakpointLocation {
    const lineNumber = expectNumber(value.lineNumber, `${label}.lineNumber`);
    const columnNumber = value.columnNumber !== undefined ? expectNumber(value.columnNumber, `${label}.columnNumber`) : undefined;
