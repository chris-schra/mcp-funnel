...
describe('Development Command Integration', () => {
  describe('MCP tool exposure', () => {
    ...
    it('should expose cmd__npm__lookup when commands enabled', async () => {
      ...
      // Verify cmd__npm__lookup is registered
      expect(toolDefinitionCache.has('cmd__npm__lookup')).toBe(true);
      expect(toolDefinitionCache.has('cmd__npm__search')).toBe(true);

      const lookupTool = toolDefinitionCache.get('cmd__npm__lookup');
      const searchTool = toolDefinitionCache.get('cmd__npm__search');

      expect(lookupTool).toBeDefined();
      expect(lookupTool!.serverName).toBe('development-commands');
      expect(lookupTool!.tool.name).toBe('cmd__npm__lookup');
      ...
      expect(searchTool!.tool.name).toBe('cmd__npm__search');
    ...
    it('should respect filtering configuration', async () => {
      ...
      const _configWithExpose: ProxyConfig = {
        servers: [],
        commands: { enabled: true, list: ['npm'] },
        exposeTools: ['cmd__npm__lookup'], // Only expose lookup tool
      };
      ...
      const _configWithHide: ProxyConfig = {
        servers: [],
        commands: { enabled: true, list: ['npm'] },
        hideTools: ['cmd__npm__search'], // Hide search tool
      };
      ...
      expect(toolDefinitionCacheHide.has('cmd__npm__lookup')).toBe(true);
      expect(toolDefinitionCacheHide.has('cmd__npm__search')).toBe(true);
    });
    ...
    it('should not expose commands when disabled', async () => {
      ...
      const toolDefinitionCache = proxy.toolDefinitionCache;
      expect(toolDefinitionCache.has('cmd__npm__lookup')).toBe(false);
      expect(toolDefinitionCache.has('cmd__npm__search')).toBe(false);
    });
