...
- use tool `add_comment_to_pending_review` to add comments to specific lines or files for pending review
- use tool `submit_pending_pull_request_review` to submit the review when done - body should be "Review Summary"
  you created after following `.haino/flows/review.md`
...
- repo - Repository name (string)
- pullNumber - PR number (number)
- event - Review action: "APPROVE", "REQUEST_CHANGES", or "COMMENT" (enum)
